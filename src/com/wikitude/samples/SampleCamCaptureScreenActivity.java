		...
		return new ArchitectUrlListener() {
			...
			public boolean urlWasInvoked(final String uriString) {
				...
				if ("markerselected".equalsIgnoreCase(invokedUri.getHost())) {
					...
					poiDetailIntent.putExtra(SamplePoiDetailActivity.EXTRAS_KEY_POI_IMAGE_URL, String.valueOf(invokedUri.getQueryParameter("imageurl")) );
					...
					SampleCamCaptureScreenActivity.this.architectView.captureScreen(ArchitectView.CaptureScreenCallback.CAPTURE_MODE_CAM_AND_WEBVIEW, new CaptureScreenCallback() {
						...
						public void onScreenCaptured(final Bitmap screenCapture) {
							...
							try {
								...
								final String chooserTitle = "Compartir";
								...
								SampleCamCaptureScreenActivity.this.runOnUiThread(new Runnable() {
									...
									public void run() {
										...
										Toast.makeText(SampleCamCaptureScreenActivity.this, "¡Ups! Error inesperado. Por favor, intente de nuevo. , " + e, Toast.LENGTH_LONG).show();	
									}
								});
							}
						}
				...
				// pressed snapshot button. check if host is button to fetch e.g. 'architectsdk://button?action=captureScreen', you may add more checks if more buttons are used inside AR scene
				else if ("trackID".equalsIgnoreCase(invokedUri.getHost())) {

					String url = invokedUri.toString();
					//Analytics
					String idTracked = url.substring(url.indexOf("trackID?id=")+11,url.indexOf("&tag="));
					String tagNameTracked = url.substring(url.indexOf("&tag=")+5);
					
					System.out.println(idTracked);
					System.out.println(tagNameTracked);
					 // Get tracker.
